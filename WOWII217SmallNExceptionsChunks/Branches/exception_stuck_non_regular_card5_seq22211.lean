...
theorem exception_stuck_non_regular_card5_seq22211 
    ...
    (hFinTrans : ∃ G_fin : SimpleGraph (Fin 5), [DecidableRel G_fin.Adj] ∧ G_fin.Connected ∧ G_fin.degreeSequence = G.degreeSequence ∧ (Traceable G_fin → Traceable G))
    (hSeqG : G.degreeSequence = [2, 2, 2, 1, 1])
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  rcases hFinTrans with ⟨G_fin, _, conn_fin, hSeqEq, hTrans⟩
  have hSeq : G_fin.degreeSequence = [2, 2, 2, 1, 1] := by
    simpa [hSeqEq] using hSeqG
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_22211_closes
    (encodeUpper5 G_fin)
    (connectedUpper_of_connected_graphOfUpper5 _ (by
      rwa [graphOfUpper_encodeUpper5]
    ))
    (by
      rw [← hSeq]
      exact fixedDegreeSequenceUpper_encodeUpper5_of_degreeSequence G_fin
    )
  ...
  rwa [graphOfUpper_encodeUpper5] at traceEncoded
