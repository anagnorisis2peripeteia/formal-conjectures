...
open Classical in
theorem exception_stuck_non_regular_card6_seq533322 
    ...
    (hFinTrans : ∃ e : Fin 6 ≃ V, List.ofFn (fun v : Fin 6 => G.degree (e v)) = [5, 3, 3, 3, 2, 2]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [5, 3, 3, 3, 2, 2]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_6 _ H connected_degreeSequence_533322_closes hconn hds)
    connected
    hFinTrans
