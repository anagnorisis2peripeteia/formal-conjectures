...
theorem exception_stuck_non_regular_card7_seq4444222 
    ...
    (hFinTrans : ∃ e : Fin 7 ≃ V, List.ofFn (fun v : Fin 7 => G.degree (e v)) = [4, 4, 4, 4, 2, 2, 2]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [4, 4, 4, 4, 2, 2, 2]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_7 _ H connected_degreeSequence_4444222_closes hconn hds)
    connected
    hFinTrans
