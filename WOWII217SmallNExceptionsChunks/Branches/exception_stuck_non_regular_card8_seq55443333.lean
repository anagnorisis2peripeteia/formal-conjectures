...
theorem exception_stuck_non_regular_card8_seq55443333 
    ...
    (hFinTrans : ∃ e : Fin 8 ≃ V, List.ofFn (fun v : Fin 8 => G.degree (e v)) = [5, 5, 4, 4, 3, 3, 3, 3]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [5, 5, 4, 4, 3, 3, 3, 3]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_8 _ H connected_degreeSequence_55443333_closes hconn hds)
    connected
    hFinTrans
