...
theorem exception_stuck_non_regular_card8_seq54443332 
    ...
    (hFinTrans : ∃ e : Fin 8 ≃ V, List.ofFn (fun v : Fin 8 => G.degree (e v)) = [5, 4, 4, 4, 3, 3, 3, 2]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [5, 4, 4, 4, 3, 3, 3, 2]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_8 _ H connected_degreeSequence_54443332_closes hconn hds)
    connected
    hFinTrans
