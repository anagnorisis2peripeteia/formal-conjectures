...
theorem exception_stuck_non_regular_card10_seq6666655422 
    ...
    (hFinTrans : ∃ e : Fin 10 ≃ V, List.ofFn (fun v : Fin 10 => G.degree (e v)) = [6, 6, 6, 6, 6, 5, 5, 4, 2, 2]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [6, 6, 6, 6, 6, 5, 5, 4, 2, 2]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_10 _ H connected_degreeSequence_6666655422_closes hconn hds)
    connected
    hFinTrans
