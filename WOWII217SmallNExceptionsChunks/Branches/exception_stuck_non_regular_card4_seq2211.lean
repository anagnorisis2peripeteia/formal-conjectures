...
theorem exception_stuck_non_regular_card4_seq2211 
    ...
    (hFinTrans : ∃ e : Fin 4 ≃ V, List.ofFn (fun v : Fin 4 => G.degree (e v)) = [2, 2, 1, 1]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [2, 2, 1, 1]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_4 _ H connected_degreeSequence_2211_closes hconn hds)
    connected
    hFinTrans
