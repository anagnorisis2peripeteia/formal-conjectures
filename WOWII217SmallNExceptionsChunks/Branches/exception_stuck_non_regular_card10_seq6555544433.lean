...
theorem exception_stuck_non_regular_card10_seq6555544433 
    ...
    (hSeqG : G.degreeSequence = [6, 5, 5, 5, 5, 4, 4, 4, 3, 3])
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  rcases hFinTrans with ⟨G_fin, _, conn_fin, hSeqEq, hTrans⟩
  have hSeq : G_fin.degreeSequence = [6, 5, 5, 5, 5, 4, 4, 4, 3, 3] := by
