...
open Classical in
theorem exception_stuck_non_regular_card5_seq33321 
    ...
    (hFinTrans : ∃ e : Fin 5 ≃ V, List.ofFn (fun v : Fin 5 => G.degree (e v)) = [3, 3, 3, 2, 1]) :
  ∃ a b : V, ∃ p : G.Walk a b, p.IsHamiltonian
  := by

  exact WOWII217Bridge.traceable_of_card_eq G [3, 3, 3, 2, 1]
    (fun H _ hconn hds =>
      WOWII217Bridge.certified_5 _ H connected_degreeSequence_33321_closes hconn hds)
    connected
    hFinTrans
