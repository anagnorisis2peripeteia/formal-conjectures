...
theorem exception_stuck_non_regular_card7 
  ...
  | 7 =>
have hFinTrans : ∃ G_fin : SimpleGraph (Fin 7), [DecidableRel G_fin.Adj] ∧ G_fin.Connected ∧ G_fin.degreeSequence = G.degreeSequence ∧ (Traceable G_fin → Traceable G) := sorry
rcases hFinTrans with ⟨G_fin, _, conn_fin, hSeqEq, hTrans⟩
if hSeq : G_fin.degreeSequence = [5, 4, 4, 4, 4, 2, 1] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_5444421_closes
    (encodeUpper7 G_fin)
    (connectedUpper_of_connected_graphOfUpper7 _ (by
      rwa [graphOfUpper_encodeUpper7]
    ))
    (by
      rw [← hSeq]
      exact fixedDegreeSequenceUpper_encodeUpper7_of_degreeSequence G_fin
    )
  ...
  rwa [graphOfUpper_encodeUpper7] at traceEncoded
  apply hTrans; exact traceEncoded
else if hSeq : G_fin.degreeSequence = [4, 4, 4, 4, 4, 1, 1] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_4444411_closes
    (encodeUpper7 G_fin)
    (connectedUpper_of_connected_graphOfUpper7 _ (by
      rwa [graphOfUpper_encodeUpper7]
    ))
  ...
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_4444321_closes
    (encodeUpper7 G_fin)
    (connectedUpper_of_connected_graphOfUpper7 _ (by
      rwa [graphOfUpper_encodeUpper7]
    ))
  ...
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_4444222_closes
    (encodeUpper7 G_fin)
    (connectedUpper_of_connected_graphOfUpper7 _ (by
      rwa [graphOfUpper_encodeUpper7]
    ))
  ...
  rwa [graphOfUpper_encodeUpper7] at traceEncoded
  apply hTrans; exact traceEncoded
else if hSeq : G_fin.degreeSequence = [3, 3, 3, 3, 2, 2, 2] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_3333222_closes
    (encodeUpper7 G_fin)
    (connectedUpper_of_connected_graphOfUpper7 _ (by
      rwa [graphOfUpper_encodeUpper7]
    ))
