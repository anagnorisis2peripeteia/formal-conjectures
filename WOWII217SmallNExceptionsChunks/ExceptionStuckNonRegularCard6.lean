...
theorem exception_stuck_non_regular_card6 
  ...
  | 6 =>
have hFinTrans : ∃ G_fin : SimpleGraph (Fin 6), [DecidableRel G_fin.Adj] ∧ G_fin.Connected ∧ G_fin.degreeSequence = G.degreeSequence ∧ (Traceable G_fin → Traceable G) := sorry
rcases hFinTrans with ⟨G_fin, _, conn_fin, hSeqEq, hTrans⟩
if hSeq : G_fin.degreeSequence = [4, 3, 3, 3, 2, 1] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_433321_closes
    (encodeUpper6 G_fin)
    (connectedUpper_of_connected_graphOfUpper6 _ (by
      rwa [graphOfUpper_encodeUpper6]
    ))
    (by
      rw [← hSeq]
      exact fixedDegreeSequenceUpper_encodeUpper6_of_degreeSequence G_fin
    )
  ...
  rwa [graphOfUpper_encodeUpper6] at traceEncoded
  apply hTrans; exact traceEncoded
else if hSeq : G_fin.degreeSequence = [4, 3, 3, 2, 2, 2] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_433222_closes
    (encodeUpper6 G_fin)
    (connectedUpper_of_connected_graphOfUpper6 _ (by
      rwa [graphOfUpper_encodeUpper6]
    ))
  ...
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_333311_closes
    (encodeUpper6 G_fin)
    (connectedUpper_of_connected_graphOfUpper6 _ (by
      rwa [graphOfUpper_encodeUpper6]
    ))
  ...
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_333221_closes
    (encodeUpper6 G_fin)
    (connectedUpper_of_connected_graphOfUpper6 _ (by
      rwa [graphOfUpper_encodeUpper6]
    ))
  ...
  rwa [graphOfUpper_encodeUpper6] at traceEncoded
  apply hTrans; exact traceEncoded
else if hSeq : G_fin.degreeSequence = [3, 3, 2, 2, 2, 2] then
  have hDecide := WOWII217FiniteSmallExceptions.connected_degreeSequence_332222_closes
    (encodeUpper6 G_fin)
    (connectedUpper_of_connected_graphOfUpper6 _ (by
      rwa [graphOfUpper_encodeUpper6]
    ))
