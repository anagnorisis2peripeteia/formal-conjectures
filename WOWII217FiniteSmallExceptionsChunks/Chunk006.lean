...
theorem connected_degreeSequence_333221_closes_closure_chain :
    ∀ g g1 g2 g3 g4 : BitVec 15,
      connectedUpper (n := 6) g = true →
      fixedDegreeSequenceUpper (n := 6) g
        [3, 3, 3, 2, 2, 1] = true →
      (g1 == pathClosureParallelRound (n := 6) g) = true →
      (g2 == pathClosureParallelRound (n := 6) g1) = true →
      (g3 == pathClosureParallelRound (n := 6) g2) = true →
      (g4 == pathClosureParallelRound (n := 6) g3) = true →
      completeUpper (n := 6) g4 = true := by
...
set_option maxRecDepth 100000 in
set_option maxHeartbeats 1000000000 in
theorem connected_degreeSequence_333221_closes :
    ∀ g : BitVec 15,
      connectedUpper (n := 6) g = true →
      fixedDegreeSequenceUpper (n := 6) g
        [3, 3, 3, 2, 2, 1] = true →
      completeUpper (n := 6) (pathClosureParallelRounds (n := 6) 4 g) = true := by
  intro g connected degrees
  let g1 := pathClosureParallelRound (n := 6) g
  let g2 := pathClosureParallelRound (n := 6) g1
  let g3 := pathClosureParallelRound (n := 6) g2
  let g4 := pathClosureParallelRound (n := 6) g3
  have closed := connected_degreeSequence_333221_closes_closure_chain g g1 g2 g3 g4 connected degrees
