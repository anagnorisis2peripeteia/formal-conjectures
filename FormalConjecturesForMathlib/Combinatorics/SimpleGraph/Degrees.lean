...
/-- Graffiti.pc definition 63: The minimum degree among vertices in a subset `S`. -/
noncomputable def minDegreeIn (G : SimpleGraph α) [DecidableRel G.Adj] (S : Finset α) : ℕ :=
  if h : S.Nonempty then
    (S.image (fun s => G.degree s)).min' (h.image _)
  else 0
