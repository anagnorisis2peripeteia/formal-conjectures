...
/-- Distance from a vertex to a set `S`. -/
noncomputable def distanceToSet (G : SimpleGraph α) (S : Finset α) (v : α) : ℕ :=
  if h : S.Nonempty then
    (S.image (fun s => G.dist s v)).min' (h.image _)
  else 0

/-- Graffiti.pc definition 109: average distance from a set `S`. -/
noncomputable def averageDistanceToSet (G : SimpleGraph α) (S : Finset α) : ℝ :=
  letI targets := Finset.univ.filter (fun (v : α) => distanceToSet G S v > 0)
  if targets.Nonempty then
    (∑ v ∈ targets, (distanceToSet G S v : ℝ)) / (targets.card : ℝ)
  else 0
