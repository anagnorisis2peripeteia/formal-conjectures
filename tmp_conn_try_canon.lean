...
example :
  ...
  simp (config := { maxSteps := 1000000000 }) only
    [hasHighLowEdge13, fixedDegreeSequenceUpper, canonicalPermutationBlocksPattern13,
      matchesDegreesFromUpper, degreeBitsUpper, BoolFour.increment, BoolFour.same,
      BoolFour.ofNat, connectedUpper, reachableFromZeroUpper, adjUpper, setBit, bitMask,
      maskHas, List.range, List.range.loop, List.foldl, List.all, List.any,
      List.flatMap, List.flatten, List.map, List.append, List.length_cons, List.length_nil]
