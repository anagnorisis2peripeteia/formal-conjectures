...
lemma boolFiveValueR_foldl_increment {α : Type*} (f : α → Bool) :
  ...
  | cons x xs ih =>
      ...
      · have hinit : boolFiveValueR initial < 31 := by omega
        ...
        simpa [List.foldl_cons, hfx, hinc, ih']
      · have hzero : initial.increment false = initial := by simp [BoolFive.increment]
        have htail : boolFiveValueR initial + xs.length ≤ 31 := by omega
        have ih' := ih initial htail
        simpa [List.foldl_cons, hfx, hzero, ih']

lemma boolFoldl_add_eq {α : Type*} (f : α → Bool) :
    ∀ (a : Nat) (xs : List α),
  ...
  have hfoldv :
    ...
    have hbound : boolFiveValueR bitsu + (List.range 13).length ≤ 31 := by
      ...
      have hdu : degreeUpperNat (n := 13) g u ≤ 13 := by
        ...
        omega
      simpa using (show degreeUpperNat (n := 13) g u + 13 ≤ 31 by omega)
    have hvFold :=
    ...
    have hsumv :
      ...
      have haux := boolFoldl_add_eq (f := fun w => adjUpper (n := 13) g v w) (a := boolFiveValueR bitsu)
      simpa [haux, hfoldu, degreeUpperNat, Nat.add_assoc, Nat.add_left_comm, Nat.add_comm] using
        (boolFoldl_add_eq (f := fun w => adjUpper (n := 13) g v w) 0 (List.range 13)).symm
    calc
      boolFiveValueR (degreePairBits13 g u v) =
          (List.range 13).foldl (fun count x => count + if adjUpper (n := 13) g v x then 1 else 0)
