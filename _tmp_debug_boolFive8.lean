import WOWII217Closure13Fast
import WOWII217FiniteBase

open WOWII217FiniteBase WOWII217Closure13Fast

def boolFiveValue (x : BoolFive) : Nat :=
  (if x.b0 then 1 else 0) +
    2 * (if x.b1 then 1 else 0) +
    4 * (if x.b2 then 1 else 0) +
    8 * (if x.b3 then 1 else 0) +
    16 * (if x.b4 then 1 else 0)

theorem boolFiveValue_increment (x : BoolFive) (b : Bool) (hx : boolFiveValue x < 31) :
    boolFiveValue (x.increment b) =
      boolFiveValue x + if b then 1 else 0 := by
  rcases x with ⟨x0, x1, x2, x3, x4⟩
  fin_cases x0 <;> fin_cases x1 <;> fin_cases x2 <;> fin_cases x3 <;> fin_cases x4 <;>
    fin_cases b <;> simp [boolFiveValue, BoolFive.increment] at hx ⊢

theorem test_foldl_add {α : Type*} (f : α → Bool) (xs : List α) :
    List.foldl (fun count x => count + if f x then 1 else 0) 1 xs =
      1 + List.foldl (fun count x => count + if f x then 1 else 0) 0 xs := by
  have hhom : ∀ (a : Nat) (y : α),
      (fun count x => count + (if f x then 1 else 0)) (1 + a) y =
        1 + (fun count x => count + (if f x then 1 else 0)) a y := by
    intro a y
    simp [Nat.add_assoc, Nat.add_left_comm, Nat.add_comm]
  simpa using (List.foldl_hom (f := fun n : Nat => 1 + n)
    (g₁ := fun (a : Nat) (y : α) => a + (if f y then 1 else 0))
    (g₂ := fun (a : Nat) (y : α) => a + (if f y then 1 else 0))
    (l := xs) (init := 0) hhom)

theorem test {α : Type*} (f : α → Bool) :
    ∀ (xs : List α) (initial : BoolFive),
      boolFiveValue initial + xs.length < 32 →
      boolFiveValue (xs.foldl (fun bits x => bits.increment (f x)) initial) =
        boolFiveValue initial +
          (xs.foldl (fun count x => count + if f x then 1 else 0) 0) := by
  intro xs
  induction xs with
  | nil => simp
  | cons x xs ih =>
      intro initial bound
      simp [List.foldl_cons]
      have hb : boolFiveValue initial + xs.length + 1 < 32 := by
        simpa [List.length_cons, Nat.add_assoc] using bound
      have initialLt : boolFiveValue initial < 31 := by omega
      cases h : f x with
      | false =>
          have incrementValue :
              boolFiveValue (initial.increment false) = boolFiveValue initial := by
            have hInc := boolFiveValue_increment initial false initialLt
            simpa [h] using hInc
          have ihTailBound :
              boolFiveValue (initial.increment false) + xs.length < 32 := by
            rw [incrementValue]
            omega
          have ihTail := ih (initial.increment false) ihTailBound
          rw [ihTail, incrementValue]
          simp [Nat.add_assoc, Nat.add_left_comm, Nat.add_comm]
      | true =>
          have incrementValue :
              boolFiveValue (initial.increment true) = boolFiveValue initial + 1 := by
            have hInc := boolFiveValue_increment initial true initialLt
            simpa [h] using hInc
          have ihTailBound :
              boolFiveValue (initial.increment true) + xs.length < 32 := by
            rw [incrementValue]
            omega
          have ihTail := ih (initial.increment true) ihTailBound
          rw [ihTail, incrementValue]
          have htrue : (if true = true then 1 else 0 : Nat) = 1 := by decide
          simpa [htrue, test_foldl_add, Nat.add_assoc, Nat.add_left_comm, Nat.add_comm]
