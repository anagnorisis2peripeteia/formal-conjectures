...
theorem hamiltonian_of_residue_eq_two_card_le_twelve
      ...
      · by_cases h3 : Fintype.card V = 8 ∧ G.IsRegularOfDegree 3
        ...
        · by_cases h4 : Fintype.card V = 10 ∧ G.IsRegularOfDegree 4
          ...
          · -- Stuck non-regular residue-2 classes are empty (force regular).
            exact WOWII217SmallNExceptions.exception_stuck_non_regular (G := G) (connected := connected) (hCard := hn) (hResidue := _hResidue) (hMaxDeg := hMaxDeg) (hStuck := hStuck) (hNot2 := h2) (hNot3 := h3) (hNot4 := h4)
  · let t : ℕ := Fintype.card V / 2
    ...
    · by_cases hOutOK :
      ...
      · exact WOWII217SmallNExceptions.exception_not_hOutOK (G := G) (connected := connected) (hCard := hn) (hResidue := _hResidue) (hMaxDeg := hMaxDeg) (t := t) (S := S) (hS := by intro v; simp [S]) (hBig := hBig) (hNotOutOK := hOutOK)
    · exact WOWII217SmallNExceptions.exception_not_hBig (G := G) (connected := connected) (hCard := hn) (hResidue := _hResidue) (hMaxDeg := hMaxDeg) (t := t) (S := S) (hS := by intro v; simp [S]) (hNotBig := hBig)
