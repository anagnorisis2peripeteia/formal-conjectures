...
open WOWII217FiniteBase WOWII217Finite13 WOWII217Closure

/-- Local copy of shared-degree path relation machinery -/
def local_hasHighLowEdge13 (g : BitVec 78) : Bool :=
  (List.range 7).any fun u =>
    (List.range 6).any fun offset => adjUpper (n := 13) g u (offset + 7)

/-- Local reconstruction of the degree-table packing -/
...
def local_degreeTableOfUpper13 (g : BitVec 78) : BitVec 52 :=
  BitVec.ofFnLE fun i =>
...
/-- Encode graph as upper-triangle bits directly, without extra imports. -/
def local_encodeUpper13 (G : SimpleGraph (Fin 13)) [DecidableRel G.Adj] : BitVec 78 := by
  let bits : List Bool := List.ofFn fun i : Fin 78 =>
    let edge := (upperPairs 13).getD i (0, 0)
    let u : Fin 13 := ⟨edge.1, by omega⟩
    let v : Fin 13 := ⟨edge.2, by omega⟩
    decide (G.Adj u v)
  exact BitVec.cast List.length_ofFn (BitVec.ofBoolListLE bits)

/-- Same graph as in Counter13: K7 on {0,1,2,3,4,5,7} and K6 on {6,8,9,10,11,12}. -/
...
instance : DecidableRel G.Adj := Classical.decRel _

def g : BitVec 78 := local_encodeUpper13 G
...
example : local_degreeTableConsistent13 g (local_degreeTableOfUpper13 g) = true := by
  native_decide

example : local_pathClosureParallelRel13 g g (local_degreeTableOfUpper13 g) = true := by
  native_decide
...
example :
  ...
  intro h
  have h01 : local_degreeTableConsistent13 g (local_degreeTableOfUpper13 g) = true := by
    native_decide
  have h02 : local_pathClosureParallelRel13 g g (local_degreeTableOfUpper13 g) = true := by
    native_decide
  have h03 : completeUpper (n := 13) g = true :=
    h (local_degreeTableOfUpper13 g) (by native_decide) h01 h02
  have hfalse : completeUpper (n := 13) g = false := by
    native_decide
